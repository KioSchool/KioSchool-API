...
data class AcquisitionInfo(
    ...
    fun normalized(): AcquisitionInfo {
        ...
        val trimmedContext = context?.trim()?.takeIf { it.isNotEmpty() }
        if (channel != AcquisitionChannel.ETC) return copy(channelEtc = null, context = trimmedContext)
        return copy(channelEtc = trimmedEtc, context = trimmedContext)
    }
